...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("𝙳𝙾𝙽'𝚃 𝚃𝚁𝚈 𝚃𝙾 𝙼𝙰𝙺𝙴 𝙼𝙴 𝙵𝙾𝙾𝙻 🙁 𝙹𝙾𝙸𝙽 𝚃𝙷𝙴 𝚄𝙿𝙳𝙰𝚃𝙴𝚂 𝙲𝙷𝙰𝙽𝙽𝙴𝙻", show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
        ...
        await query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                ...
                InlineKeyboardButton('⬅️ 𝗕𝗮𝗰𝗸', callback_data='photo')
             ]]
        ),)
        ...
        await query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    ...
                    [
                        InlineKeyboardButton('⬅️ 𝗕𝗮𝗰𝗸', callback_data='glitch')
                    ],
                ]
            ),
        )
        ...
        buttons = [[
            ...
            InlineKeyboardButton('𝙵𝙸𝙻𝙴-𝚂𝚃𝙾𝚁𝙴', callback_data='filestore'),
...
async def advantage_spell_chok(msg):
    ...
    if not movielist:
        k = await msg.reply("𝚃𝙷𝙸𝚂 𝙼𝙾𝚅𝙸𝙴 I𝚂 𝙽𝙾𝚃 𝚈𝙴𝚃 𝚁𝙴𝙻𝙴𝙰𝚂𝙴𝙳 𝙾𝚁 𝙰𝙳𝙳𝙴𝙳 𝚃𝙾 𝙳𝙰𝚃𝚂𝙱𝙰𝚂𝙴 💌")
        await asyncio.sleep(8)
        await k.delete()
        return
    ...
    await msg.reply("📌 𝙲𝙷𝙴𝙲𝙺 𝙰𝙽𝙳 𝚂𝙴𝙻𝙴𝙲𝚃 𝚃𝙷𝙴 𝙼𝙾𝚅𝙸𝙴 𝙵𝚁𝙾𝙼 𝚃𝙷𝙴 𝙶𝙸𝚅𝙴𝙽 𝙻𝙸𝚂𝚃 📌",
                    reply_markup=InlineKeyboardMarkup(btn))
